...
export function hasInjectionMarker(parts: Array<{ type: string; text?: string }>): boolean {
  return parts.some((p) => p.type === "text" && typeof p.text === "string" && p.text.includes(INJECT_MARKER))
}

/** 注入 part：synthetic 标记 + 幂等标记文本 + 捕获的 system 全量。 */
export function buildInjectionPart(system: string, sessionID: string, messageID: string): InjectionPart {
  return {
    id: newPartId(),
    sessionID,
    messageID,
