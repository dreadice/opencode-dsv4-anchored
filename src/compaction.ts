 ...
 * （无幂等标记即注入、判别窗口重置）。
 */
export async function compacting(ctx: CompactingCtx, sessionID: string): Promise<void> {
  ...
  await ctx.client.session.update({ path: { id: sessionID }, body: { permission: compactionRules() } })
