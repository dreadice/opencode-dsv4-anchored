import { PROBE_THROW_MESSAGE, captureProbeSystem, probeKey, type ProbeStore } from "@/probe"
...
export async function systemTransform(
  ...
  if (cached?.status === "failed" && Date.now() - cached.ts < ctx.options.probeTtlMs) return
