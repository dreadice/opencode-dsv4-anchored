...
test("TC-2-21: create 写新文件 + 已存在报错", async () => {
  ...
  const r1 = await strReplaceEditor.execute({ command: "create", path: p, file_text: "hello" }, ctx)
  assert.ok(String(r1).includes("created"))
  assert.equal(await readFile(p, "utf8"), "hello")
  await assert.rejects(
    strReplaceEditor.execute({ command: "create", path: p, file_text: "again" }, ctx),
    /already exists/,
  )
})
...
test("TC-2-20: view 文件行号格式 + view_range", async () => {
  ...
  const full = String(await strReplaceEditor.execute({ command: "view", path: p }, ctx))
  assert.ok(full.includes("    1\tl1"))
  assert.ok(full.includes("    4\tl4"))
  const ranged = String(
    await strReplaceEditor.execute({ command: "view", path: p, view_range: [2, 3] }, ctx),
  )
  assert.ok(ranged.includes("    2\tl2"))
  assert.ok(ranged.includes("    3\tl3"))
  assert.ok(!ranged.includes("l1"))
  const toEnd = String(
    await strReplaceEditor.execute({ command: "view", path: p, view_range: [3, -1] }, ctx),
  )
  assert.ok(toEnd.includes("l3") && toEnd.includes("l4") && !toEnd.includes("l1"))
})

test("TC-2-20b: view 16000 截断 + <response clipped>", async () => {
  const p = join(dir, "big.txt")
  await writeFile(p, "x".repeat(20000), "utf8")
  const out = String(await strReplaceEditor.execute({ command: "view", path: p }, ctx))
...
test("TC-2-20c: view 目录列出（非隐藏，2 层）", async () => {
  ...
  const out = String(await strReplaceEditor.execute({ command: "view", path: d }, ctx))
...
test("TC-2-22: str_replace 唯一替换 + 多/无匹配报错", async () => {
  ...
  await assert.rejects(
    strReplaceEditor.execute({ command: "str_replace", path: p, old_str: "zzz", new_str: "x" }, ctx),
    /No matches/,
  )
...
test("TC-2-23: insert 行插入", async () => {
  ...
  await writeFile(p, "a\nb\nc\n", "utf8")
  await strReplaceEditor.execute({ command: "insert", path: p, insert_line: 1, new_str: "X" }, ctx)
