...
import { createFakeClient, addSession, assistant, user, reasoning, textPart, toolPart } from "./fake-client.ts"
...
function makeCtx(): { ctx: EnsureCtx; client: ReturnType<typeof createFakeClient> } {
...
function input(sessionID: string, parts: unknown[] = [{ type: "text", text: "hi" }]): EnsureInput {
  return { sessionID, model: MODEL, messageID: "msg_user", outputParts: parts as EnsureInput["outputParts"] }
}

/** 预置探针缓存（捕获值），避免测试里跑探针。 */
function seedProbe(ctx: EnsureCtx, directory = "/proj", agent = "build"): string {
  const key = probeKey(directory, agent, MODEL.modelID)
  captureProbeSystem(ctx.probeStore, key, "SYSTEM-FULL")
  return key
}

const sentinel = (pattern: string): Rule => ({ permission: STAGE_PERMISSION, pattern, action: "allow" })
...
test("TC-2-1: pristine 首轮 → 注入 + seeded", async () => {
  ...
  assert.ok(String((parts[0] as { text: string }).text).includes(INJECT_MARKER), "注入 part 应在最前")
  const s = client._sessions.get("ses_1")!
  assert.ok(s.permission.some((r) => r.permission === "*" && r.action === "deny"))
  assert.ok(s.permission.some((r) => r.permission === STAGE_PERMISSION && r.pattern === "seeded"))
})

test("TC-2-2: 探针成功（缓存写入 + 会话清理）", async () => {
  const { ctx, client } = makeCtx()
  addSession(client, { id: "ses_1" })
  client.setProbeHandler(async (id) => {
...
test("TC-2-4: 探针并发去重（两次调用只 create 一次）", async () => {
  ...
  client.setProbeHandler(async (id) => {
    captureProbeSystem(ctx.probeStore, probeKey("/proj", "build", MODEL.modelID), "S")
    throw new Error("DSV4 probe: capture complete")
  })
  await Promise.all([ensureState(ctx, input("ses_1")), ensureState(ctx, input("ses_2"))])
...
test("TC-2-5: 注入幂等（seeded 后再来不重复注入）", async () => {
  ...
  client._messages.set("ses_1", [user("msg_1", [textPart(`${INJECT_MARKER}\nSYSTEM`)])])
...
test("TC-2-6: seeded + 历史 assistant 消息 → 解锁", async () => {
  ...
  assert.ok(s.permission.some((r) => r.permission === "str_replace_editor" && r.action === "deny"))
  assert.ok(s.permission.some((r) => r.permission === "*" && r.action === "allow"), "agent ruleset 应覆盖")
})
...
test("TC-2-7: 解锁幂等（unsealed 不再重复追加；判别独立进行）", async () => {
  ...
  client._messages.set("ses_1", [assistant("msg_1", [reasoning("Let me check the config.")])])
  const res = await ensureState(ctx, input("ses_1"))
...
test("TC-2-8: 判别通过 → verified", async () => {
  ...
  seedProbe(ctx)
  addSession(client, { id: "ses_1", permission: [sentinel("unsealed")] })
  client._messages.set("ses_1", [assistant("msg_1", [reasoning("We need to fix the build.")])])
  const res = await ensureState(ctx, input("ses_1"))
...
test("TC-2-9: 判别 giveup（N=3 条不符，warn 一次）", async () => {
  ...
  assert.equal(client._logs.filter((l) => l.msg.includes("verify.giveup")).length, 1)
  const res2 = await ensureState(ctx, input("ses_1"))
  assert.equal(res2.action, "giveup")
  assert.equal(client._logs.filter((l) => l.msg.includes("verify.giveup")).length, 1, "giveup 去重只 warn 一次")
})
...
test("TC-2-11: bypass 稳定（failed TTL 内全旁路）", async () => {
  ...
  addSession(client, { id: "ses_1" })
  const key = probeKey("/proj", "build", MODEL.modelID)
  ctx.probeStore.map.set(key, { status: "failed", error: "boom", ts: Date.now() })
  const res = await ensureState(ctx, input("ses_1"))
  assert.equal(res.action, "bypass")
  const s = client._sessions.get("ses_1")!
...
test("TC-2-12: resume——重启后从 ruleset 恢复（seeded + 历史信号 → 解锁）", async () => {
  ...
  seedProbe(ctx)
  addSession(client, { id: "ses_1", permission: [sentinel("seeded")] })
  client._messages.set("ses_1", [assistant("msg_1", [reasoning("We need to.")])])
  const res = await ensureState(ctx, input("ses_1"))
  assert.equal(res.action, "unlock")
})
...
test("TC-2-15: subagent（parentID）独立处理", async () => {
  ...
  assert.equal(res.action, "seeded")
  assert.ok(client._logs.some((l) => l.subagent === true), "日志应标记 subagent")
})

test("TC-2-16: 门控不命中 → 全旁路（原生）", async () => {
  const { ctx, client } = makeCtx()
  addSession(client, { id: "ses_1" })
  const res = await ensureState(ctx, { ...input("ses_1"), model: { providerID: "anthropic", modelID: "claude-3.5" } })
...
test("TC-2-8b: reasoning 无标记、text 有 we → 拼接后通过", async () => {
  ...
  addSession(client, { id: "ses_1", permission: [sentinel("unsealed")] })
  client._messages.set("ses_1", [
    assistant("msg_1", [reasoning("Hmm, need to inspect the repo."), textPart("We need to do it.")]),
  ])
  const res = await ensureState(ctx, input("ses_1"))
  assert.equal(res.action, "verify")
})
