...
const r = (permission: string, pattern: string, action: Rule["action"]): Rule => ({
  permission,
  pattern,
  action,
})
...
test("TC-1-10: findLast 后写覆盖（seeded → verified 取最后）", () => {
  assert.equal(getStage([sentinel("seeded"), r("*", "*", "deny"), sentinel("verified")]), "verified")
})

test("TC-1-11: 有 deny * 但无哨兵 → pristine（非插件会话）", () => {
  assert.equal(getStage([r("*", "*", "deny"), r("bash", "*", "allow")]), "pristine")
})
...
test("TC-1-13: extractSessionDenies 只取 deny 且排除插件自身 deny *", () => {
  ...
  assert.deepEqual(extractSessionDenies(rules), [r("str_replace_editor", "*", "deny"), r("task", "*", "deny")])
})
...
test("TC-1-15: unlockRules 保留 explore 只读 ruleset", () => {
  const explore = [r("*", "*", "deny"), r("read", "*", "allow"), r("bash", "*", "allow")]
...
test("TC-1-16: compactionRules 含 7 工具 + 哨兵 seeded", () => {
  ...
  const allowed = rules.filter((x) => x.action === "allow").map((x) => x.permission)
  ...
  assert.ok(rules.some((x) => x.permission === "external_directory" && x.action === "allow"))
