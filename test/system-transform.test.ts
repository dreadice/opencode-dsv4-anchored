...
function makeCtx() {
  ...
  const logger = makeLogger(client.app.log.bind(client.app), { debugEnabled: false })
  return {
    client,
    probeStore,
    logger,
...
test("TC-2-18: 探针会话 → 捕获 + throw（禁词规避）", async () => {
  ...
  let thrown = ""
  try {
    await systemTransform(ctx, { sessionID: "ses_probe_1", model: MODEL }, output)
  } catch (e) {
    thrown = String(e instanceof Error ? e.message : e)
  }
  assert.ok(thrown.includes("capture complete"), `应 throw 探针消息: ${thrown}`)
...
test("TC-2-19b: 门控不命中 → 放行", async () => {
  ...
  await systemTransform(
    ctx,
    { sessionID: "ses_1", model: { providerID: "anthropic", modelID: "claude-3.5" } },
    output,
  )
...
test("compaction 回退：含哨兵会话 → 追加回退规则 + warn", async () => {
  ...
  for (const tool of ["read", "glob", "grep", "edit", "todowrite", "question"]) {
    assert.ok(s.permission.some((r) => r.permission === tool && r.action === "allow"), `missing ${tool}`)
  }
