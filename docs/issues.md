...
- **状态**：`fixed`（通过 `skipSubagents: true` 可配置跳过；默认仍走原流程）
- **现象**：
...
- **解决方案**：
  - 新增配置项 `skipSubagents`（默认 `false`，保持原有行为）。
  - 设为 `true` 后，子代理会话会跳过插件处理：不锚定、不注入、不替换 system、
    不触发 round2。
  - 当前默认仍保留“子代理也走锚定流程”的行为，由用户按需开启跳过。
