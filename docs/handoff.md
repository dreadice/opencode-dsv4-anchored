...
当前状态：**研究完成、决策已定案（round-4 探针捕获 + round-5 失败旁路/日志/
严格 minimal 工具对/D11 备选 + dsh README 偏差核对 + round-6 compaction 触发点
与重注入 + round-7 状态机 v3：seeded/unsealed/verified + N=3 判别）、代码尚未
实现**（`src/index.ts` 还是旧骨架，含 provider hook 注册 deepseek 模型，需重写）。
