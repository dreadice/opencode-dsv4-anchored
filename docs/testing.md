...
```ts
const buildRuleset = [{permission:"*",action:"allow",pattern:"*"},
  {permission:"doom_loop",action:"ask",pattern:"*"}]
const exploreRuleset = [{permission:"*",action:"deny",pattern:"*"},
  {permission:"read",action:"allow",pattern:"*"}, {permission:"bash",action:"allow",pattern:"*"}]
const session = {id:"ses_1", directory:"/proj", parentID:undefined, agent:"build",
  model:{providerID:"opencode",modelID:"deepseek-v4-flash-free"},
  permission: [] as Rule[]}   // 初始空
const messages = [{info:{role:"assistant"}, parts:[{type:"reasoning",text:"We need to..."},
