...
| TC-3-11 | 锚定轮（D13 全链路） | `opencode serve` + HTTP API 发消息（v4-pro / flash-free，见 cli-testing.md §5.1）                                                       | 日志链：`chat.message stage=pristine injectSource=anchor` → 锚定回复 we 风格 → `unlock` → `round2.sent` → 轮 2 用 bash/glob/read 完成真实任务 → 后续消息 `verify.passed`（哨兵 verified） | ✅ 2026-08-16 实测（serve 模式）；`opencode run` 单次模式轮 2 不执行（CLI dispose 限制） |
